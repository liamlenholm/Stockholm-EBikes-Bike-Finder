...
List<GeoCoordinate> StationsLongLat = new List<GeoCoordinate>();
StationsLongLat.Add(
StationsLongLat.Add(new GeoCoordinate { Url = "https://stockholmebikes.se/map/detail/c0ce6e46-8340-475e-88c4-0085cb132904", Longitude = 18.107227164370393, Latitude = 59.306721187745076 });
StationsLongLat.Add(new GeoCoordinate { Url = "https://stockholmebikes.se/map/detail/40b879f6-1fee-40a0-8c8b-6b9ce245a41f", Longitude = 18.107164446181546, Latitude = 59.31170451854383 });
/*
...
var coords2 = new coord() { lat = 59.5, lon = 19.6 };
Findclose(coords2);

static void Findclose(coord coord)
{
    var coords3 = new GeoCoordinate(59.340499, 18.090652);

    var coords = new[] { 
        new coord() { lat = 59.340499, lon = 18.090652, lol = "test" },
        new coord() { lat = 59.440499, lon = 20.090652,  lol = "test2"}
    };

    var closest = coords.Select(p => new GeoCoordinate(p.lat, p.lon))
                       .OrderBy(x => x.GetDistanceTo(coords3))
                       .First();
    ...
    foreach (var item in coords)
    {
        ...
        if (item.lat.ToString() == closeval[0].Replace(".", ",") && item.lon.ToString() == closeval[1].Replace(".", ","))
        {
            
           Console.WriteLine(item.lol);
        }
    }
...
//var sortedcords = StationsLongLat.OrderBy(x => x.GetDistanceTo(crd)).ToList();

//Console.WriteLine(sortedcords);
